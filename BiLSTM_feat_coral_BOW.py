...
def load_CES_data(use_audio=True, use_visual=True, use_linguistic=False, targets=[0,1,2]):
    ...
    if use_audio:
        train_DE_x = np.concatenate( (train_DE_x, load_features1(path_features= '../eGEMAPS_avg_std_coral_BOW/', partition='Train_FE', num_inst=num_train_DE, max_seq_len=max_seq_len) ), axis=2)
        devel_DE_x = np.concatenate( (devel_DE_x, load_features1(path_features= '../eGEMAPS_avg_std_coral_BOW/', partition='Devel_HU', num_inst=num_devel_DE, max_seq_len=max_seq_len) ), axis=2)
    ...
    if use_visual:
        train_DE_x = np.concatenate( (train_DE_x, load_features1(path_features='../MSA_std_coral_BOW/', partition='Train_FE', num_inst=num_train_DE, max_seq_len=max_seq_len) ), axis=2)
        devel_DE_x = np.concatenate( (devel_DE_x, load_features1(path_features='../MSA_std_coral_BOW/', partition='Devel_HU', num_inst=num_devel_DE, max_seq_len=max_seq_len) ), axis=2)
    ...
    _                       , train_DE_y = load_labels(path_labels='../labels/', partition='Train_FE', num_inst=num_train_DE, max_seq_len=max_seq_len, targets=targets)
    devel_DE_labels_original, devel_DE_y = load_labels(path_labels='../labels/', partition='Devel_HU', num_inst=num_devel_DE, max_seq_len=max_seq_len, targets=targets)
...
#     path_output = 'predictions/'  # To store the predictions on the test partitions
    ...
    batch_size    = 2      # Full-batch: 34 sequences
 ...
 #   dropout       = 0.1      # Dropout
    ...
    df.to_csv(path_results + 'BilSTM_feat_coral_BOW.csv', index=None)
