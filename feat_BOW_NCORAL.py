...
##### MSF features
    chinese_path = '../Chinese_eGMAPS_BOW/'
    feat_path = "../eGEMAPS_BOW/"
    out_path1 = "../eGEMAPS_BOW_Ncoral/"
   ...
   # standadization
    ...
    for filename2 in target_data_path:
        DS = pd.read_csv(os.path.join(feat_path, filename2), sep=';',header=None).values
    ...
    chinese_path = '../chinise_MSf_BOW/'
    feat_path = "../MSF_BOW/"
    out_path1 = "../MSF_BOW_Ncoral/"
    target_data_path = fnmatch.filter(os.listdir(feat_path), 'Devel_*')
    source_data_path = fnmatch.filter(os.listdir(feat_path), 'Train_*')
    print(len(source_data_path), len(target_data_path))
    source_data_path.sort()
