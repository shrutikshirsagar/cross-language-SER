...
def load_CES_data(use_audio=True, use_visual=True, use_linguistic=False, targets=[0,1,2]):
    ...
    if use_audio:
        train_DE_x = np.concatenate( (train_DE_x, load_features(path_features= '../eGEMAPS_avg_std_SCL/', partition='Train_DE', num_inst=num_train_DE, max_seq_len=max_seq_len) ), axis=2)
        devel_DE_x = np.concatenate( (devel_DE_x, load_features(path_features= '../eGEMAPS_avg_std_SCL/', partition='Devel_FE', num_inst=num_devel_DE, max_seq_len=max_seq_len) ), axis=2)
    ...
    if use_visual:
        train_DE_x = np.concatenate( (train_DE_x, load_features(path_features='../MSF_std_SCL/', partition='Train_DE', num_inst=num_train_DE, max_seq_len=max_seq_len) ), axis=2)
        devel_DE_x = np.concatenate( (devel_DE_x, load_features(path_features='../MSF_std_SCL/', partition='Devel_FE', num_inst=num_devel_DE, max_seq_len=max_seq_len) ), axis=2)
...
#     path_output = 'predictions/'  # To store the predictions on the test partitions
 ...
 #   dropout       = 0.1      # Dropout
    ...
    df.to_csv(path_results + 'BilSTM_feat_SCL.csv', index=None)
